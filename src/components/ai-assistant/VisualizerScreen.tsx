...
const loadingMessages = ["Analyzing window dimensions...", "Mapping room perspective...", "Detecting light sources...", "Selecting fabric texture...", "Rendering realistic textures...", "Applying final touches..."];
const VisualizerScreen = ({
  ...
  const generateVisualization = useCallback(async () => {
    ...
    try {
      const [_, result] = await Promise.all([minLoadingTime, generateBlindsVisualization(preferences.photo, {
  ...
  return <div className="container mx-auto px-4 py-6 md:py-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 animate-slide-up">
            <div className="relative bg-card rounded-2xl overflow-hidden shadow-strong">
              {/* View Toggle */}
              {generatedImage && !isLoading && <div className="absolute top-4 left-4 z-20 bg-card/90 backdrop-blur-sm rounded-lg p-1 flex text-xs font-medium shadow-medium">
                  <button onClick={() => setViewMode('ai')} className={`px-3 py-1.5 rounded-md transition-all flex items-center gap-1.5 ${viewMode === 'ai' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-secondary/50'}`}>
                    <Sparkles className="h-3 w-3" /> AI View
                  </button>
                  <button onClick={() => setViewMode('original')} className={`px-3 py-1.5 rounded-md transition-all ${viewMode === 'original' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-secondary/50'}`}>
                    Original
                  </button>
                </div>}
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-primary/10">
                    {preferences.photo && <div className="absolute inset-0 z-0 opacity-40 blur-md">
                    ...
                    <div className="relative z-10 flex flex-col items-center p-8 rounded-2xl bg-card/95 backdrop-blur-md shadow-strong max-w-sm text-center mx-4">
                      ...
                      <div className="w-40 h-1.5 bg-secondary/20 rounded-full overflow-hidden mt-4">
                        <div className="h-full bg-secondary animate-pulse rounded-full" style={{
              ...
              <div className="absolute inset-0 flex items-center justify-center bg-destructive/5">
                    <div className="flex flex-col items-center p-8 rounded-2xl bg-card shadow-strong max-w-sm text-center mx-4">
                      <div className="w-16 h-16 bg-destructive/10 rounded-full flex items-center justify-center mb-4">
                        {error.type === 'NO_WINDOW' ? <ImageOff className="h-8 w-8 text-destructive" /> : <AlertTriangle className="h-8 w-8 text-destructive" />}
                      </div>
              ...
              // Image Display
              <>
                    <img src={displayImage} alt="Your room visualization" className="w-full h-full object-cover transition-all duration-500" />
                    
                    {!preferences.photo && <div className="absolute bottom-20 left-1/2 -translate-x-1/2 bg-card/90 backdrop-blur-sm text-primary px-3 py-1.5 rounded-full shadow-medium text-xs font-medium z-10">
                        Viewing Sample Room
                      </div>}
                  </>}
                ...
                {/* Actions */}
                {!isLoading && !error && <div className="absolute top-4 right-4 flex gap-2 z-10">
                    <Button variant="close" size="iconSm" onClick={() => setIsFavorite(!isFavorite)} className={`bg-card/80 backdrop-blur-sm ${isFavorite ? "text-red-500" : ""}`}>
                ...
                {!isLoading && !error && <div className="absolute bottom-4 left-4 right-4 z-10">
                    <div className="bg-card/95 backdrop-blur-sm rounded-xl p-3 shadow-medium">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="w-6 h-6 rounded-full border-2 border-border shadow-sm" style={{
              ...
              {!isLoading && preferences.photo && <div className="p-4 bg-tertiary border-t border-border">
                  <Button variant="ai" className="w-full" onClick={handleRegenerate} disabled={isLoading}>
          ...
          animationDelay: "0.2s"
            ...
            <div className="bg-card rounded-xl p-4 shadow-soft">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                ...
                return <button key={style.id} onClick={() => handleStyleChange(style)} className={`
                        px-2.5 py-1.5 rounded-lg border-2 transition-all flex items-center gap-1.5
                        ${selectedStyle.id === style.id ? "border-primary bg-secondary/20" : "border-border hover:border-secondary"}
                      `}>
            ...
            <div className="bg-card rounded-xl p-4 shadow-soft">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                {colorOptions.map(color => <button key={color.id} onClick={() => handleColorChange(color)} className={`
                      relative w-10 h-10 rounded-full transition-all duration-200
                      ${selectedColor.id === color.id ? "ring-[3px] ring-blue-500 ring-offset-2 ring-offset-card scale-110" : "hover:scale-105"}
                    `} style={{
            ...
            <div className="bg-card rounded-xl p-4 shadow-soft">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                ...
                return <button key={opacity.id} onClick={() => handleOpacityChange(opacity)} className={`
                        px-2.5 py-1.5 rounded-lg border-2 transition-all flex items-center gap-1.5
                        ${selectedOpacity.id === opacity.id ? "border-primary bg-secondary/20" : "border-border hover:border-secondary"}
                      `}>
                      <IconComponent className="w-3.5 h-3.5 text-muted-foreground" />
        ...
        {/* CTA */}
        <div className="mt-8 text-center animate-scale-in" style={{
        animationDelay: "0.4s"
      }}>
